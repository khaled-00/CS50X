...
char* indexes(const char* path)
{
    if (path != NULL) 
    {
        ...
        char* directory = malloc(sizeof(char) * (strlen(path) + 15));
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    BYTE* buffer = malloc(sizeof(BYTE) * 2);
    
    int n = 0;
    while(fread(buffer + n, sizeof(BYTE), 1, file) != 0)
    {
        n++;
...
{ 
	...
	// spaces counter to make sure there's only 2 spaces in line
    ...
    // a loop to stop before the ? for the abs path
    int counter;
    for (counter = 0; counter < strlen(word)  + 1; counter++)
    {
        if (word[counter] == '?')
        break;
    }
    
	strncpy(abs_path, word, counter);
	
    // substringing the query from the request-target
    // searching for the '?' and putting the query (including the '?'') inside temp
    char* temp;
    temp = strrchr(word, '?');
    // if there is no '?' or no query after the '?', assign query to ""
    if (temp == NULL || strlen(temp) == 1)
        query = "";
    else
    {
        // copying temp into query from the second char (q) to leave away the '?''
        strcpy(query, temp+1);
    }
